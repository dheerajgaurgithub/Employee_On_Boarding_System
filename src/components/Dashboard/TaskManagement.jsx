...
const TaskManagement = ({ role }) => {
  ...
  const myTasks = (() => {
    const assignedToMe = getTasksByUser(currentUser._id) || [];
    const assignedByMe = getTasksByAssigner(currentUser._id) || [];

    if (role === 'employee') return assignedToMe;
    if (role === 'hr') return [...assignedToMe, ...assignedByMe];
    if (role === 'admin') return assignedByMe;
    return [];
  })();
...
// Place this after the TaskManagement component
function renderSubmissionDetails(task) {
  // TODO: Implement actual submission details rendering if needed
  return null;
}
